...
class GTJCActionServer : public rclcpp::Node {
 public:
  ...
  explicit GTJCActionServer(std::shared_ptr<rclcpp::Node> move_group_node,
    ...
    // initialize action server callbacks
 ...
 private:
  ...
  void execute(const std::shared_ptr<GoalHandleGTJC> goal_handle) {
    ...
    result->success = move_group_->move() == moveit::planning_interface::MoveItErrorCode::SUCCESS;

    if (rclcpp::ok()) {
      goal_handle->succeed(result);
      RCLCPP_INFO(this->get_logger(), "Go to joint config finished.");
    }
  }
